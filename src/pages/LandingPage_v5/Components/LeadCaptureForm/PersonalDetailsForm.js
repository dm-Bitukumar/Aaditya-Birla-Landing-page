...
const PersonalDetailsForm = ({
  ...
  const handleProfessionChnage = (value) => {
    console.log(value);
  ...
  useEffect(() => {
    const incomeValid =
      (professionType === "salaried" &&
        monthlyIncome.replace(/,/g, "").match(/^\d+$/)) ||
  ...
  useEffect(() => {
    if (formData) {
      ...
      setAnnualIncome(formData.annual_income || "");
    }
  }, []);
      ...
      <div className="personal-details-container-v3">
        ...
        <div className="personal-details-input">
          ...
          <FormSelectStyle2
            ...
            onChange={handleProfessionChnage}
            onBlur={() => {
              if (!professionType) {
          ...
          {professionType === "salaried" ? (
            <FormInputStyle2
              ...
              value={
                monthlyIncome
                  ? Number(monthlyIncome.replace(/,/g, "")).toLocaleString(
                      "en-IN"
                    )
                  : ""
              }
            ...
            <FormInputStyle2
              label="Anuual Income (As Per ITR)"
              value={
                annual_income
                  ? Number(annual_income.replace(/,/g, "")).toLocaleString(
                      "en-IN"
                    )
                  : ""
              }
              ...
              onChange={(e) => {
                ...
                if (/^\d*$/.test(raw)) {
                  // Ensure only numbers
                  setAnnualIncome(raw); // Store raw value (unformatted)
                  if (errors.annual_income) {
                    setErrors((prev) => ({ ...prev, annual_income: false }));
                  }
                }
              }}
              onBlur={() => {
                if (!annual_income.match(/^\d+$/)) {
                  setErrors((prev) => ({ ...prev, annual_income: true }));
                } else {
                  // Format on blur
                  setMonthlyIncome(
                    Number(annual_income).toLocaleString("en-IN")
                  );
                }
              }}
              isValid={!errors.annual_income}
              errorMessage="Enter valid income"
            />
          ) : null}
          {/* <FormInputStyle2
