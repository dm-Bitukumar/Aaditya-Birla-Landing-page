                    ...
                    <Dropdown.Menu style={{ maxHeight: '180px', overflowY: 'scroll', position: 'absolute' }}>
                        <div className="form-floating">
                            ...
                            {filteredOptions.map((option) => (
                                <Dropdown.Item key={option.value} onClick={() => handleOptionSelect(option.value)}>
                                    {option.label}
                                </Dropdown.Item>
                            ))}
                            {filteredOptions.length === 0 && <Dropdown.Item disabled>No matching options</Dropdown.Item>}
                        </div>
                    </Dropdown.Menu>
                </Dropdown>
                {!isValid && (
